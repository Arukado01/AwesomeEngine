...
inline const sf::Font &GetDefaultFont() {
  static const sf::Font font("Content/Fonts/Relidux.otf");
  return font;
};
...
inline bool IsOutsideWindowRight(const sf::Shape &shape) {
  return shape.getPosition().x >
         gConfig.windowSize.x - shape.getGlobalBounds().size.x / 2;
}
...
inline bool IsOutsideWindowBottom(const sf::Shape &shape) {
  return shape.getPosition().y >
         gConfig.windowSize.y - shape.getGlobalBounds().size.y / 2;
}

inline bool IsOutsideWindow(sf::Vector2f point) {
  return point.x < 0 || point.y < 0 || point.x >= gConfig.windowSize.x ||
         point.y >= gConfig.windowSize.y;
}

inline std::optional<sf::FloatRect> Intersects(const sf::Shape &shape1,
...
inline bool Contains(const sf::CircleShape &circle, sf::Vector2f point) {
  return DistanceSquared(circle.getPosition(), point) <=
         std::pow(circle.getRadius(), 2);
}
